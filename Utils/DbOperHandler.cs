...
namespace CJHSJ_OilWebView.Utils
{
    ...
    public abstract class DbOperHandler : IDisposable
    {
        ...
        {
            DataSet ds = this.GetDataSet(ProcedureName);
            return ds.Tables[0];
        }
        /// <summary>
        /// 根据当前指定的SqlCmd获取DataSet。如果ConditionExpress不为空则会将其清空，所以条件表达式需要包含在SqlCmd中。
        ...
        /// <param name="proName"></param>
        /// <returns></returns>
        public IDataReader GetReader(string ProcedureName)
        {
            this.cmd.CommandText = ProcedureName;
            this.cmd.CommandType = System.Data.CommandType.StoredProcedure;//指定是存储过程
            this.GenParameters();
            return this.cmd.ExecuteReader();
        }
